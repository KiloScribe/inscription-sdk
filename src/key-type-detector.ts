...
export function detectKeyTypeFromString(
  ...
  if ((has0xPrefix || isRawHex64 || isRawHex128) && isHex) {
    let firstType: KeyType = 'ed25519';
    let secondType: KeyType = 'ecdsa';

    // Convention: 32-byte (64-char) hex with 0x prefix is ECDSA
    if (isRawHex64 && has0xPrefix) {
      firstType = 'ecdsa';
      secondType = 'ed25519';
    }
    ...
    try {
      ...
      return { detectedType: firstType, privateKey };
    } catch (firstError) {
      try {
        const privateKey =
          secondType === 'ecdsa'
        ...
        throw new Error(
          `Failed to parse private key as either ${firstType} or ${secondType}: ${firstError}`,
        );
      }
    }
  }
